...
def got_gpio_interrupt(channel):  
    ...
    if debug: 
        print("Got GPIO interrupt on pin",channel, end=' ')
        # get name of pin
        print(list({k for (k,v) in list(globals().items()) if v==channel })[0])
...
def delayedReenableInterrupts():
    ...
    for channel in callbacksToReenable:
        if debug: 
            print("reenable",channel,time.ctime(), end=' ')
            # get name of pin
            print(list({k for (k,v) in list(globals().items()) if v==channel })[0])
...
def checkpoint(signum, frame):
    ...
    if debug: print(s)
    try:
        f = urllib.request.urlopen(s, None, 3)
        xml = f.read()
        f.close()
